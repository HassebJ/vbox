...
var SQLMap 	= require('./library/sqlmap');
...
module.exports = function(options, db) {
	...
	this.connect = function(callback){
		...
		connection.connect(function(error) {
			if(error){ 
				var date = new Date();
				throw new Error('MySQL Connect Error('+ date.toUTCString() +'): ' + error);
				setTimeout(function(){ that.connect(callback); }, 2000); 
			} else {
		...
		connection.on('error', function(error) {
			...
			if(error.code === 'PROTOCOL_CONNECTION_LOST') { // Connection to the MySQL server is usually
				that.connect(callback);                          	// lost due to either server restart, or a
			} else {                                      	// connnection idle timeout (the wait_timeout
				throw new Error('MySQL Standard Error(' 	// server variable configures this)
				+ date.toUTCString() +'): ' + error);	                      
			}
		});
		return connection;
	}
...
*/
var default_functions = ['create', 'save', 'saveMore', 'update', 'delete', 'query', 'connect', 'get', 'getBy', 'fields'];
...
mysql_wrapper = function(request, response, mysql, custom_map){
	...
	function MYSQL(query, callback, async){
		...
		mysql.query(query, function(error, rows){
			...
			callback(rows, error_handler, query);
		});
	}
