...
function Authenticate(setup){
	if(!setup)      var setup = {};
	if(!setup.table) 	setup.table 	= 'vbox.accounts';
	if(!setup.username) setup.username 	= 'email';
	...
	Auth.login = function(response, session, redirect){
            ...
            mysql.accounts.update('email', this.username, { online:1 }, function() {
	...
	Auth.logout = function(request, response, mysql, redirect, callback){
		...
		mysql.accounts.update('session', request.cookies.id, { session: 0, online:0 }, function(){
...
module.exports = function(app){
	...
	function get_gravatar(text, size, d) {
	...
	// ====================( Account / Delete ) ====================
	app.post('/accounts/delete', function(request, response, mysql){
		if(isset(response.head.account.id)){
			// check password
...
//						fs.unlinkSync(app.public + '/uploads/avatar/tiny/' 		+ user.avatar);
	...
	// ====================( Account / Recover / Password ) ====================
	app.get(/\/accounts\/recover\/([^\/]+)\/?/, function(request, response, mysql){
		response.data.reset_code = request.params[1];
		if(isset(response.data.reset_code)){
			response.data.page = 'recover';
			response.finish();
		} else {
			app.notFoundHandler(request, response, 'reset_code not found');
		}
	});
	
	app.post(/\/accounts\/recover\/([^\/]+)\/?/, function(request, response, mysql){
		request.demand('new_password');
		request.demand('new_password_again');
		request.demand('new_password').equals(request.body.new_password_again);
		if(isset(request.params[1])){
			if(request.passed){
				mysql.accounts.update('reset_code', request.params[1], { 
					'reset_code': 0, 
					...
					if(rows.affectedRows){
						...
						request.error('reset_code', 'is invalid');
						finish();
					}
				});
			} else {
			...
			mysql.accounts.get('email', request.body.recovery_email, function(rows){
				if(rows && rows.length){
					...
					mysql.accounts.update('id', rows[0].id, { reset_code: reset_code }, function(){
						...
						if(events.recover_notify){ 
							events.recover_notify(request, response, mysql); 
						} else {
							response.success();
							mysql.end();
						}
	...
	app.get(/\/accounts\/verify\/([^\/]+)\/?(success\/?)?/i, function(request, response, mysql){
		...
		if(isset(key)){
			...
			if(!isset(success)){
				// find user by key
				mysql.accounts.get('id', key, function(rows){
					// key accepted: activate user
					if(rows && rows[0]){
						var account = rows[0];
						account.session = Auth.createSession(account.password);
						mysql.accounts.update('id', key, { activated: 1 }, function(){
...
//        if(!isset(success)){
//            // find user by key
//            mysql.accounts.get('acticated', key, function(rows){
//                // key accepted: activate user
//                if(rows && rows[0]){
//                    var account = rows[0];
//                    account.session = Auth.createSession(account.password);
